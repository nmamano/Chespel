...
% Let us start with a simple example written in \chespel.
...
% \chespel\ only has two data types: \emph{integer} and \emph{Boolean}.
% It does not have any complex data structures such as arrays or structs
% (that is why the language is so simple).

% \chespel\ is a typeless programming language, i.e., any variable can have
...
% \chespel\ does not have global variables. All the variables are local within
...
% \chespel\ can only read from the standard input and write to the standard
...
% \chespel\ has a rich set of logical, relational and arithmetic operators.
